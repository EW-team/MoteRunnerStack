...
namespace Mac_Layer
{
	...
	internal class MacUnassociatedState : MacState
	{
		...
		{
			this.mac.radio.setShortAddr (0xFFFF);
			this.panId = panId;
			this.trackBeacon();
		}
...
//							this.mac.radio.setPanId (this.panId, false);
						...
						this.mac.radio.transmit (Radio.ASAP | Radio.TXMODE_CCA, assRequest, 0,
...
//					Logger.flush (Mote.INFO);
		...
		public override int onTxEvent(uint flags, byte[] data, uint len, uint info, long time){
			if(this.duringSuperframe)
				this.mac.radio.startRx (Radio.ASAP | Radio.RXMODE_NORMAL, 0, 0);
			uint modeFlag = flags & Device.FLAG_MODE_MASK;		
			if (modeFlag == Radio.FLAG_ASAP || modeFlag == Radio.FLAG_EXACT || modeFlag == Radio.FLAG_TIMED) {
				switch (data [0] & 0x07) {
					case Radio.FCF_CMD:
		...
		public override void onTimerEvent (byte param, long time)
		{
			if (param == Mac.MAC_SLEEP) {
				...
				if (this.mac.radio.getState () == Radio.S_RXEN)
					this.mac.radio.stopRx ();
				this.mac.timer.setParam (Mac.MAC_WAKEUP);
				this.mac.timer.setAlarmTime (time + this.beaconInterval -
		...
		{ // nei diagrammi è espresso anche come scanBeacon()	
			...
			this.mac.radio.startRx (Radio.ASAP|Radio.RXMODE_NORMAL, 0, Time.currentTicks () + this.aScanInterval);
		}
	}
}
